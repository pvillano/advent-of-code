...
def reparse() -> tuple[tuple[int, tuple[tuple[int, int], ...]], ...]:
    ...
    itoa = sorted(edges_str.keys())
    atoi = {ch: idx for idx, ch in enumerate(itoa)}
    ret = []
    for name in itoa:
        flow_rate = flows_str[name]
        children = edges_str[name]
        new_children = []
        for child in children:
            edge = atoi[child]
            weights_key = min(name, child), max(name, child)
            weight = weights_str[weights_key]
            new_children.append((edge, weight))
        ret.append((flow_rate, tuple(sorted(new_children))))
    return tuple(ret)
...
def part2():
    ...
    @cache
    def dp(me=0, ellie=0, used=0, my_time_left=26, ellie_time_left=26):
        assert my_time_left >= ellie_time_left
        best = 0
        flow_rate_me, edges_me = mega_dict[me]
        if flow_rate_me > 0 and not (used & 1 << me) and my_time_left > 1:
